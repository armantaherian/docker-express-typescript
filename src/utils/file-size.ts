...
export default function filesize(bytes: number, options?: any) {
  ...
  options.calculate = function (spec: any) {
    ...
    var fixed: any = result.toFixed(options.fixed)
    var suffix
  ...
  options.to = function (unit: any, spec: any) {
    var algorithm = equals(spec, 'si') ? 1e3 : 1024
  ...
  options.human = function (spec: any) {
